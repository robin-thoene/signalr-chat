        ...
        <div className="dropdown-end dropdown">
            ...
            <ul tabIndex={0} className="dropdown-content menu rounded-box mt-5 w-52 border border-base-200 bg-base-100 shadow-xl">
                <li className={`${i18n.language === 'en' && 'disabled'}`}>
                    {i18n.language !== 'en' ? (
                        <Link aria-label={t('languageButtonEnAriaLabel')} className="flex py-3 px-6" href={router.asPath} locale={'en'}>
                            {t('en')}
                        </Link>
                    ) : (
                        <div className="flex py-3 px-6">{t('en')}</div>
                    )}
                </li>
                <li className={`${i18n.language === 'de' && 'disabled'}`}>
                    {i18n.language !== 'de' ? (
                        <Link aria-label={t('languageButtonDeAriaLabel')} className="flex py-3 px-6" href={router.asPath} locale={'de'}>
                            {t('de')}
                        </Link>
                    ) : (
                        <div className="flex py-3 px-6">{t('de')}</div>
                    )}
                </li>
            </ul>
        </div>
